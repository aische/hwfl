...
import Pml.Ast.Name (Slug (..))
import Pml.Parse.Markdown (MdFence (..), MdHeading (..))
...
-- next heading of equal or higher level, excluding fenced @pml@ blocks.
...
buildSections srcLines headings fences =
  ...
  where
    ...
    pmlFences = filter isPmlFence fences
    ...
    mkSection i h =
      Section
        { secSlug = computeSlug (mhText h),
          secLevel = mhLevel h,
          secTitle = mhText h,
          secBody = T.strip (sliceWithoutFences srcLines pmlFences contentStart contentEnd)
        }
      where
