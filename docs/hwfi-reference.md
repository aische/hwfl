...
| Growing `builtin/list-*` / `builtin/json-*` in Haskell         | Belongs in hwfl stdlib             |
...
1. Quote the _behaviour_ or invariant needed (e.g. “confirm freezes par pool”).
2. Point at hwfi files for reference.
3. Require the new implementation to sit behind hwfl’s host-op / frame APIs.
