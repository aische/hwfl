module Hwfl.Runtime.SimpleCodingAgentSpec (spec) where
...
projectRoot = "examples/simple-coding-agent"
...
spec :: Spec
spec = describe "simple-coding-agent example" $ do
  ...
  it "builds a tiny Python project after loading the python skill (mock LLM)" $
    withSystemTempDirectory "hwfl-simple-coding-agent" $ \tmp -> do
      result <- checkProject projectRoot
      case result of
        Left err -> expectationFailure (show err)
        Right cpr -> do
                ...
                runLoadedModule
                  RunOptions
                    { roWorkspace = tmp,
                      ...
                      roRunId = Just "simple-coding-agent",
                      roEntry = modulePath,
                      roMode = StepRun,
